...
def dump_tensor(f, name: str, tensor: torch.Tensor, ggml_type: GGMLType):
    ...
    #     f.write(struct.pack("i", tensor.shape[tensor.ndim - 1 -i]))
    # f.write(struct.pack("i", ggml_type.value))
    # tensor data
...
def dump_state_dict(f, weight_names, state_dict, quantization_bit, ggml_type):
    ...
    for name in tqdm(weight_names, desc="Processing model states"):
        ...
        if tensor.ndim == 2:
            ...
            else: tensor_ggml_type = ggml_type
        else:
            # 1d weight: convert it to float32
            # assert tensor.ndim == 1, f"unimplemented: name={name},tensor.ndim={tensor.ndim}"
            if tensor.ndim == 1:
                tensor = tensor.float()
                tensor_ggml_type = GGMLType.F32
            else:
                if tensor.ndim==3 and tensor.shape[1]==1 and tensor.shape[0]==1:
    ...
    log = tabulate(tensor_info, headers=["name", "shape", "dtype"], tablefmt="psql")
...
class CogVLM2Converter(BaseConverter):
    MODEL_TYPE = ModelType.COGVLM2
    @staticmethod
    def dump_config(f, config, ggml_type):
        config_values = [
            ggml_type.value,
            ...
            config.vocab_size,
            config.num_hidden_layers,
            config.num_multi_query_heads,
        ]
        f.write(struct.pack("i" * len(config_values), *config_values))

    # @staticmethod
    # def dump_tokenizer(f, tokenizer):
    #     serialized_model_proto = tokenizer2SentencePiece(tokenizer)
    #     f.write(struct.pack("i", len(serialized_model_proto)))
    #     f.write(serialized_model_proto)
    @staticmethod
    def dump_model(f, model, ggml_type):
        assert  model.state_dict()["model.embed_tokens.weight"] is not None, \
        "unimplemented: lm_head weight must be tied to input embedding"
        weight_names = ["model.embed_tokens.weight"]
        for i in range(model.config.num_hidden_layers):
            # weight_names+=[f"model.layers.{i}.self_attn.rotary_emb.inv_freq",]
            weight_names+=[f"model.layers.{i}.self_attn.vision_expert_query_key_value.weight",]
            weight_names+=[f"model.layers.{i}.self_attn.vision_expert_query_key_value.bias",]
            weight_names+=[f"model.layers.{i}.self_attn.vision_expert_dense.weight",]
            weight_names+=[f"model.layers.{i}.self_attn.language_expert_query_key_value.weight",]
            weight_names+=[f"model.layers.{i}.self_attn.language_expert_dense.weight",]

            weight_names+=[f"model.layers.{i}.mlp.language_mlp.gate_proj.weight",]
            weight_names+=[f"model.layers.{i}.mlp.language_mlp.up_proj.weight",]
            weight_names+=[f"model.layers.{i}.mlp.language_mlp.down_proj.weight",]
            weight_names+=[f"model.layers.{i}.mlp.vision_mlp.gate_proj.weight",]
            weight_names+=[f"model.layers.{i}.mlp.vision_mlp.up_proj.weight",]
            weight_names+=[f"model.layers.{i}.mlp.vision_mlp.down_proj.weight",]

            weight_names+=[f"model.layers.{i}.input_layernorm.weight",]
            weight_names+=[f"model.layers.{i}.post_attention_layernorm.weight",]
        
        weight_names+=[f"model.norm.weight",]
        weight_names+=["model.vision.patch_embedding.proj.weight",]
        weight_names+=["model.vision.patch_embedding.proj.bias",]
        weight_names+=["model.vision.patch_embedding.position_embedding.weight",]
        weight_names+=["model.vision.patch_embedding.cls_embedding",]

        for i in range(model.config.vision_config["num_hidden_layers"]):
            weight_names +=[f"model.vision.transformer.layers.{i}.input_layernorm.weight"]
            weight_names +=[f"model.vision.transformer.layers.{i}.input_layernorm.bias"]
            weight_names +=[f"model.vision.transformer.layers.{i}.attention.query_key_value.bias"]
            weight_names +=[f"model.vision.transformer.layers.{i}.attention.query_key_value.weight"]
            weight_names +=[f"model.vision.transformer.layers.{i}.attention.dense.bias"]
            weight_names +=[f"model.vision.transformer.layers.{i}.attention.dense.weight"]
            weight_names +=[f"model.vision.transformer.layers.{i}.mlp.fc1.bias"]
            weight_names +=[f"model.vision.transformer.layers.{i}.mlp.fc1.weight"]
            weight_names +=[f"model.vision.transformer.layers.{i}.mlp.fc2.bias"]
            weight_names +=[f"model.vision.transformer.layers.{i}.mlp.fc2.weight"]
            weight_names +=[f"model.vision.transformer.layers.{i}.post_attention_layernorm.bias"]
            weight_names +=[f"model.vision.transformer.layers.{i}.post_attention_layernorm.weight"]

        weight_names+=["model.vision.linear_proj.linear_proj.weight",]
        weight_names+=["model.vision.linear_proj.norm1.weight",]
        weight_names+=["model.vision.linear_proj.norm1.bias",]
        weight_names+=["model.vision.linear_proj.dense_h_to_4h.weight",]
        weight_names+=["model.vision.linear_proj.gate_proj.weight",]
        weight_names+=["model.vision.linear_proj.dense_4h_to_h.weight",]
        weight_names+=["model.vision.conv.weight",]
        weight_names+=["model.vision.conv.bias",]
        weight_names+=["model.vision.boi",]
        weight_names+=["model.vision.eoi",]

        weight_names+=["lm_head.weight"]
        dump_state_dict(f, weight_names, model.state_dict(), None, ggml_type)
...
def convert(f: BinaryIO, model_name_or_path: str, lora_model_name_or_path: Optional[str] = None, dtype: str = "q4_0"):
    ...
    model = auto_model_class.from_pretrained(model_name_or_path, torch_dtype=TORCH_TYPE, trust_remote_code=True, low_cpu_mem_usage=True)
    ...
    CogVLM2Converter.convert(f,model,tokenizer,ggml_type)

def main():
    parser = argparse.ArgumentParser("cogvlm-convert")
    parser.add_argument(
        "-i",
        "--model_name_or_path",
        default="/opt/data/private/LLModel/cogvlm2-llama3-chinese-chat-19B",
        type=str,
    ...
    parser.add_argument(
        "-o", "--save_path", default="cogvlm-ggml.bin", type=Path, help="Path to save the generated GGML model"
    )
    parser.add_argument(
        "-t",
        "--type",
